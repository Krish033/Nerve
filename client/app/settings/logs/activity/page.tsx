...
import { useActivityLogs } from '../_partials/imports/queries';
...
export default function ActivityLogsPage() {
  ...
  const { data, isLoading: loading } = useActivityLogs({
    q: debouncedSearch,
    page,
    limit
  });
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-border/40 pb-10">
        <Heading 
          ...
          description="Real-time system telemetry and audit trails" 
          className="mb-0" 
        />
        
        <div className="flex items-center gap-8 py-4 px-6 border border-border/40 bg-muted/5 rounded-2xl">
          <div className="space-y-1 text-right">
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/30 leading-none">Buffer Status</p>
            <div className="flex items-center gap-2 justify-end">
              <div className="h-2 w-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)] animate-pulse" />
              <span className="text-[13px] font-semibold text-primary">Scanning Matrix</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-16">
        {loading ? (
          <div className="py-24 flex flex-col items-center justify-center gap-6 opacity-40">
            ...
            <p className="text-[12px] font-bold tracking-widest uppercase">Synchronizing...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="py-24 flex flex-col items-center justify-center gap-6 border-2 border-dashed border-border/40 rounded-3xl opacity-10">
            ...
            <p className="text-[13px] font-bold tracking-widest uppercase">Buffer Empty</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50 text-muted-foreground">
                    <th className="px-6 py-4 font-bold uppercase tracking-widest text-[11px]">Action</th>
                    ...
                    <th className="px-6 py-4 font-bold uppercase tracking-widest text-[11px]">User / IP</th>
                  ...
                  {logs.map((log: ActivityLog) => (
                    <tr key={log.id} className="hover:bg-muted/20 transition-colors group">
                      <td className="px-6 py-4 font-medium">
                        <span className="text-foreground group-hover:text-primary transition-colors">{log.action}</span>
                        <span className="ml-2 text-[10px] text-muted-foreground uppercase tracking-widest px-2 py-0.5 bg-muted rounded-full">
                          {log.module}
                        </span>
                      </td>
